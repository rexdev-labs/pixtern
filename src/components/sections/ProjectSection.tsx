...
import { ProjectSectionType } from "@/types/projectSection";
...
export default function ProjectSection() {
    ...
    const { data: dataProject } = useFetch<ProjectSectionType>(
        "/homepage?populate[projectSection][populate][section][populate]=*&populate[projectSection][populate][projects][populate]=*"
    );

    const titleProject = dataProject?.data.projectSection.section.title;
    const [splitFirst, rest] = useMemo(() => {
        return splitTextFirst(titleProject ?? "");
    }, [titleProject]);
    const subtitleProject =
        dataProject?.data.projectSection.section.description;

    const projects =
        dataProject?.data.projectSection.projects.map((t) => ({
            title: t.title,
    ...
    useGSAP(
        () => {
            if (!titleProject || !subtitleProject) return;
            if (!splitFirst || !rest) return;
