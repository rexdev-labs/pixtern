...
export default function PersonCard({
  ...
  className,
  href = "#",
}: Readonly<PersonCardProps & HTMLAttributes<HTMLDivElement>>) {
  const cardContainerRef = useRef(null);

  const { contextSafe } = useGSAP(undefined, { scope: cardContainerRef.current! });
    ...
    >
      <Link href={href}>
        ...
        >
          ...
          >
            <VStack
              backgroundImage={`url(${process.env.NEXT_PUBLIC_BASE_URL}${backgroundImage.url})`}
            ...
            >
              <Image
                src={`${process.env.NEXT_PUBLIC_BASE_URL}${personImage.url}`}
                alt={`${name} char`}
                boxSize={{
                  base: "107px",
                  sm: "122px",
